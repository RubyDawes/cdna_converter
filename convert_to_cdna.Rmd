...
```{r}
# example input file- only required columns are tx_id (matching the gtf file you use for the cdna conversion), genomic coordinate and strand (chromosome is implied by the transcript). your file can have any other columns as well, these are just the ones required for the data.table join to work
...
converted = cdna_converter[to_convert, on = .(tx_id,strand), allow.cartesian = TRUE][start <= genomic_coordinate & end >= genomic_coordinate]
...
converted = converted[, .(tx_id, genomic_coordinate, strand, cdna_position)]
